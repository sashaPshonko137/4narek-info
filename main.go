...
func adjustPrice(item string) {
	...
	if sales >= cfg.NormalSales {
		...
		if currentItemCount + inventoryCount + sales > allowedStock {
			if currentItemCount + sales < cfg.NormalSales || (buys < cfg.NormalSales && inventoryFreeSlots - buys >= cfg.NormalSales) {
				mutex.Unlock()
				return				
			}
			newPrice -= cfg.PriceStep
			...
			if ratio == 0.75 {
				ratio = 0.8
			} else if buys < cfg.NormalSales && inventoryCount + currentItemCount < cfg.NormalSales {
				newPrice += cfg.PriceStep
				if newPrice > cfg.MaxPrice {
					newPrice = cfg.MaxPrice
				}
